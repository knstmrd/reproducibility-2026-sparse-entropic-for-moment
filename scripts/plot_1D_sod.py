...
The plot assumes that the moment being plotted is the 2,0,0 one

Example:
    python plot_1d_sod.py --files a.h5 b.h5 --labels "upwind" "upwind_lw" \\
        --moment 2,0,0 --timestep -1 --length 1.0 --xmin 0.3 --xmax 0.7 \\
...
def main():
    ...
    for i, (path, label) in enumerate(zip(args.files, args.labels)):
        ...
        ax_ux.plot(x[m], ux[m], label=rf"{label}", **style)
    ...
    ax_ux.set_title("$x$-velocity", fontsize=label_size)
    ax_ux.set_ylabel(r"$u_x$", fontsize=label_size)
    ax_T.set_title("Temperature", fontsize=label_size)
    ax_T.set_ylabel(r"$T$", fontsize=label_size)

    m_abc_str = r"$M_{" + f"{a}{b}{c}" + r"} / \rho$" 
    ax_m.set_title(rf"Temperature in $x$-direction", fontsize=label_size)
    ...
    fig.suptitle(f"1D Sod shock tube: {t_txt}, {args.titletext}", fontsize=label_size)
    fig.tight_layout()
